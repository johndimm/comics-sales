...
export async function GET(_: NextRequest, { params }: { params: { id: string } }) {
  ...
  const sold = db
    .prepare(
      `SELECT pse.rank, mc.id as comp_id, mc.title, mc.issue, mc.price, mc.shipping, mc.sold_date,
              mc.grade_numeric, mc.grade_company, mc.is_raw, mc.is_signed, mc.match_score, mc.url
  ...
  const active = db
    .prepare(
      `WITH ranked AS (
       ...
       SELECT comp_id,title,issue,price,shipping,sold_date,grade_numeric,grade_company,is_raw,is_signed,match_score,url
  ...
  const soldEvidence = sold.map((r) => ({ ...r, is_raw: compIsRaw(r) }));
  const activeEvidence = active.map((r) => ({ ...r, is_raw: compIsRaw(r) }));
